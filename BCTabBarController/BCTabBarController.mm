...
@synthesize defaultTabIndex,viewControllers, tabBar, selectedTab, selectedViewController, tabBarView;
...
- (void)viewDidLoad {
	...
	self.tabBar = [[[BCTabBar alloc] initWithFrame:CGRectMake(0, self.view.bounds.size.height - TAB_BAR_HEIGHT, 
															  self.view.bounds.size.width, TAB_BAR_HEIGHT)]
				   autorelease];
...
- (void)loadTabs {
	...
	self.defaultTabIndex = (self.defaultTabIndex < self.viewControllers.count)?self.defaultTabIndex:0;
	[self.tabBar selectTabAtIndex:self.defaultTabIndex animated:NO];
}
